...
const PrimitivaTicket = mongoose.model('PrimitivaTicket');
const PrimitivaYear = mongoose.model('PrimitivaYear');
 ...
 * @apiSampleRequest /primitiva/tickets
 */
exports.findAllTickets = async (req, res) => {
  ...
  try {
    const total = await PrimitivaTicket.countDocuments().exec();

    const tickets = await PrimitivaTicket.find(filters, null, options).exec();
...
exports.findTicketById = async (req, res) => {
  ...
  try {
    const ticket = await PrimitivaTicket.findById(id).exec();
 ...
 * @apiVersion 1.0.0
 *
 * @apiSampleRequest /primitiva/years
 */
exports.findAllYears = async (req, res) => {
  try {
    const tickets = await PrimitivaYear.find().exec();
 ...
 * @apiVersion 1.0.0
 *
 * @apiSampleRequest /primitiva/years/:id
 *
...
exports.findYearById = async (req, res) => {
  ...
  try {
    const year = await PrimitivaYear.findById(id).exec();
 ...
 * @apiSampleRequest /primitiva/historical/occurrencesByResult
 */
exports.findOccurrencesByResult = async (req, res) => {
  ...
  try {
    ...
    const results = await PrimitivaTicket.aggregate(pipeline).exec();
    ...
    const data = await PrimitivaTicket.aggregate(pipeline).exec();
    result.data = data;
 ...
 * @apiParam {String} [sort_type] Sentido de la ordenación de registros. Los posibles valores son "asc" y "desc".
 * Por defecto se ordenan descendentemente.
 * @apiSampleRequest /primitiva/historical/occurrencesByResultWithReimbursement
 */
 ...
 * Por defecto se ordenan descendentemente.
 *
 * @apiSampleRequest /primitiva/historical/occurrencesByNumber
 */
 ...
 * @apiParam {String} [sort_type] Sentido de la ordenación de registros. Los posibles valores son "asc" y "desc".
 * Por defecto se ordenan descendentemente.
 * @apiSampleRequest /primitiva/historical/occurrencesByReimbursement
 */
 ...
 * Por defecto se ordenan descendentemente.
 *
 * @apiSampleRequest /primitiva/historical/lastDateByNumber
 */
...
/**
 * @api {get} /primitiva/historical/lastDateByReimbursement Consulta de fecha de última aparición por reintegro en histórico de Primitiva
 * @apiName GetPrimitivaLastDateByReimbursement
 * @apiGroup PrimitivaHistorical
 *
 * @apiDescription Recurso para la consulta de fecha de última aparición por reintegro en histórico de Primitiva.
 *
 * @apiVersion 1.2.0
 *
 * @apiParam {Number} page Número de página a consultar. Por defecto se establece a 1.
 ...
 * Por defecto se ordenan descendentemente.
 *
 * @apiSampleRequest /primitiva/historical/lastDateByReimbursement
 */
